...
namespace MhtBuilder
{
    ...
    internal class WebClientEx
    {
        ...
        private System.Text.Encoding DetectEncoding(string ContentTypeHeader, byte[] ResponseBytes)
        {
            ...
            s = Regex.Match(ContentTypeHeader, "charset=([^;\"'/>]+)", RegexOptions.IgnoreCase).Groups[0].ToString().ToLower();
            encoding = CharsetToEncoding(s);

            //-- if we can't get it from header, try the body bytes forced to ASCII
            if (encoding == null)
            {
                s = Regex.Match(System.Text.Encoding.ASCII.GetString(ResponseBytes), "<meta[^>]+content-type[^>]+charset=([^;\"'/>]+)", RegexOptions.IgnoreCase).Groups[0].ToString().ToLower();
                encoding = CharsetToEncoding(s);
                if (encoding == null)
                {
        ...
        public void GetUrlData(string Url, DateTime ifModifiedSince)
        {
            ...
            if (wresp.Headers.Get("Content-Encoding") == null)
            {
                ContentEncoding = HttpContentEncoding.None;
            }
            else
            {
                switch (wresp.Headers.Get("Content-Encoding").ToLower())
                {
                    case "gzip":
            ...
            _ContentLocation = wresp.Headers.Get("Content-Location");
            ...
            _DetectedContentType = wresp.Headers.Get("Content-Type");
        ...
        private byte[] Decompress(byte[] b, HttpContentEncoding CompressionType)
        {
            ...
            switch (CompressionType)
            {
                case HttpContentEncoding.Deflate:
                    s = new ICSharpCode.SharpZipLib.Zip.Compression.Streams.InflaterInputStream(new MemoryStream(b), new ICSharpCode.SharpZipLib.Zip.Compression.Inflater(true));
                    break;
                case HttpContentEncoding.Gzip:
                    s = new ICSharpCode.SharpZipLib.GZip.GZipInputStream(new MemoryStream(b));
                    break;
                default:
                    return b;
            }
        ...
        public class ExtendedBinaryReader : BinaryReader
        {
            ...
            public byte[] ReadToEnd(int InitialLength)
            {
                ...
                int Read = 0;
                int Chunk = this.BaseStream.Read(Buffer, Read, Buffer.Length - Read);
                while (Chunk > 0)
                {
                    Read = Read + Chunk;
                    ...
                    Chunk = this.BaseStream.Read(Buffer, Read, Buffer.Length - Read);
                }
